...
```
pip3 install devtrans psutil tqdm
```
...
sandhi\_vicchedika\_vedic.py can also be run in the same way. It additionally removes svaras and special characters from the input.

To run pada\_vishleshika.py:

```
python3 pada_vishleshika.py <input_encoding> <output_encoding> <segmentation_mode> <result_mode> [-t text] [-i input_file] [-o output_file]
```

Example:
```
python3 pada_vishleshika.py WX WX sent best -t 'rAmO gacCawi'
```
